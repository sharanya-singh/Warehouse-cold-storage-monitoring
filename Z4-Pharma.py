# Enhanced Z4-Pharma.py with User-Adjustable Threshold Configuration
...
def setup_threshold_configuration(zone_id="Z4-Pharma"):
    ...
    defaults = default_ranges.get(zone_id, {'temp_min': -5, 'temp_max': -2, 'humidity_min': 50, 'humidity_max': 60})
    ...
    with st.sidebar:
        ...
        alert_sensitivity = st.selectbox(
            ...
            index=3,  # Default to Critical for pharma (most sensitive)
        ...
        data_quality_threshold = st.slider(
            ...
            min_value=80,
            max_value=100,
            value=99,  # Highest standard for pharma
            step=1,
...
def load_data_for_pharma():
    try:
        ...
        return df[df['zone_id'] == 'Z4-Pharma'].copy()
    except FileNotFoundError:
        st.error("Data file 'simulated_warehouse_data_30min_demo.csv' not found!")
        return pd.DataFrame()

@st.cache_data
def get_pharma_forecast():
    return {'forecasted_temp': [-2.1, -2.5, -2.8, -3.0, -3.2, -3.0, -2.7, -2.4, -2.2, -2.0]}

# === MAIN PAGE CONTENT ===
st.set_page_config(page_title="Z4-Pharma Dashboard", layout="wide", page_icon="💊")

user_config = setup_threshold_configuration("Z4-Pharma")
setup_enhanced_auto_refresh()

st.title("💊 Z4-Pharma Monitoring Dashboard")
st.markdown("**Real-time temperature and humidity monitoring with customizable thresholds**")
st.write("---")

df_pharma = load_data_for_pharma()
if df_pharma.empty:
    st.error("Unable to load data for Z4-Pharma zone.")
    st.stop()

if 'humidity' in df_pharma.columns:
    df_pharma['humidity'] = df_pharma['humidity'].apply(lambda x: x if not pd.isna(x) else np.nan)

zone_data = display_threshold_aware_metrics(df_pharma, "Z4-Pharma", user_config)
...
st.markdown("**10-step temperature prediction for Z4-Pharma:**")

forecast_data = get_pharma_forecast()
...
with col3:
    ...
    st.download_button("📥 Export Data", data=zone_data_csv, 
                      file_name=f"Z4-Pharma_custom_config_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                      mime="text/csv", use_container_width=True)

st.write("---")
st.markdown(f"""
<div style="text-align: center; color: #666; padding: 15px; background: #f8f9fa; border-radius: 10px;">
    <p><strong>💊 Z4-Pharma Monitoring System</strong> | Custom Configuration Active</p>
