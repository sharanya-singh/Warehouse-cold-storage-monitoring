# Enhanced Z3-Produce.py with User-Adjustable Threshold Configuration
...
def setup_threshold_configuration(zone_id="Z3-Produce"):
    ...
    defaults = default_ranges.get(zone_id, {'temp_min': 8, 'temp_max': 12, 'humidity_min': 80, 'humidity_max': 95})
    ...
    with st.sidebar:
        ...
        alert_sensitivity = st.selectbox(
            ...
            index=2,  # Default to High for produce (more sensitive)
        ...
        data_quality_threshold = st.slider(
            ...
            min_value=80,
            max_value=100,
            value=98,  # Higher standard for produce
            step=1,
    ...
    return {
        'temp_min': temp_range[0], 'temp_max': temp_range[1], 'warning_buffer': warning_buffer,
        'humidity_min': humidity_range[0], 'humidity_max': humidity_range[1],
        'alert_sensitivity': alert_sensitivity, 'data_quality_threshold': data_quality_threshold
    }
...
def apply_custom_alert_logic(df, config):
    ...
    enhanced_df['custom_alert_status'] = enhanced_df.apply(
        lambda row: calculate_alert_status(row['temperature'], row.get('humidity')), axis=1
    )
...
def get_zone_status_color_custom(temperature, alert_count, config):
    ...
    if temp_min <= temperature <= temp_max and alert_count == 0:
        return {
            'status': 'optimal', 'bg_color': 'linear-gradient(135deg, #d4edda 0%, #c3e6cb 50%, #b3d7c1 100%)',
            'border_color': '#28a745', 'text_color': '#155724', 'status_emoji': '🟢', 'status_text': 'Optimal'
        }
    elif (temp_min - warning_buffer <= temperature <= temp_max + warning_buffer) and alert_count <= 2:
        return {
            'status': 'warning', 'bg_color': 'linear-gradient(135deg, #fff3cd 0%, #ffeaa7 50%, #fdcb6e 100%)',
            'border_color': '#ffc107', 'text_color': '#856404', 'status_emoji': '🟡', 'status_text': 'Warning'
        }
    else:
        return {
            'status': 'critical', 'bg_color': 'linear-gradient(135deg, #f8d7da 0%, #f5b7b1 50%, #e74c3c 100%)',
            'border_color': '#dc3545', 'text_color': '#721c24', 'status_emoji': '🔴', 'status_text': 'Critical'
        }

def display_threshold_aware_metrics(df, zone_id="Z3-Produce", config=None):
    ...
    <div style="background: {status_info['bg_color']}; padding: 20px; border-radius: 15px; border-left: 6px solid {status_info['border_color']}; box-shadow: 0 4px 8px rgba(0,0,0,0.15); margin-bottom: 20px;">
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                ...
                <p style="margin: 5px 0 0 0; color: {status_info['text_color']}; opacity: 0.8;">Current: {current['temperature']:.1f}°C{config_text} • {recent_alerts} active alerts</p>
            </div>
    ...
    with col1:
        ...
        st.metric(f"🌡️ Current Temperature {temp_status}", f"{current['temperature']:.1f}°C", 
...
def setup_enhanced_auto_refresh():
    ...
    with st.sidebar:
        ...
        if auto_refresh:
            refresh_interval = st.selectbox("📡 Update frequency:", options=[3, 5, 10, 30, 60], 
...
def load_data_for_produce():
    try:
        ...
        return df[df['zone_id'] == 'Z3-Produce'].copy()
    except FileNotFoundError:
        st.error("Data file 'simulated_warehouse_data_30min_demo.csv' not found!")
        return pd.DataFrame()

@st.cache_data
def get_produce_forecast():
    return {'forecasted_temp': [10.2, 10.5, 10.4, 10.1, 9.8, 9.9, 10.0, 10.3, 10.6, 10.5]}

# === MAIN PAGE CONTENT ===
st.set_page_config(page_title="Z3-Produce Dashboard", layout="wide", page_icon="🥬")

user_config = setup_threshold_configuration("Z3-Produce")
setup_enhanced_auto_refresh()

st.title("🥬 Z3-Produce Monitoring Dashboard")
st.markdown("**Real-time temperature and humidity monitoring with customizable thresholds**")
st.write("---")

df_produce = load_data_for_produce()
if df_produce.empty:
    st.error("Unable to load data for Z3-Produce zone.")
    st.stop()

if 'humidity' in df_produce.columns:
    df_produce['humidity'] = df_produce['humidity'].apply(lambda x: x if not pd.isna(x) else np.nan)

zone_data = display_threshold_aware_metrics(df_produce, "Z3-Produce", user_config)
...
if not chart_data.empty:
    ...
    base = alt.Chart(chart_data).encode(
        ...
        tooltip=[alt.Tooltip('timestamp:T', title='Timestamp'), alt.Tooltip('temperature:Q', title='Temperature (°C)', format='.2f')]
    )
    ...
    temp_line = base.mark_line(color=status_info['border_color'], strokeWidth=4, 
        ...
        y=alt.Y('temperature:Q', axis=alt.Axis(title='Temperature (°C)', titleColor=status_info['border_color']), 
...
st.markdown("**10-step temperature prediction for Z3-Produce:**")

forecast_data = get_produce_forecast()
...
with col3:
    ...
    st.download_button("📥 Export Data", data=zone_data_csv, 
                      file_name=f"Z3-Produce_custom_config_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                      mime="text/csv", use_container_width=True)

st.write("---")
st.markdown(f"""
<div style="text-align: center; color: #666; padding: 15px; background: #f8f9fa; border-radius: 10px;">
    <p><strong>🥬 Z3-Produce Monitoring System</strong> | Custom Configuration Active</p>
